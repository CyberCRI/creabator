 ...
 * @return array
 */
function wiki_prepare_form_vars($page = null, $parent_guid = 0) {
 ...
 */
function wiki_prepare_parent_breadcrumbs($page) {
 ...
 */
function wiki_register_navigation_tree($container) {
	if (!$container) {
		return;
	}
	...
	foreach ($top_pages as $page) {
		elgg_register_menu_item('wiki_nav', array(
			'name' => $page->getGUID(),
			'text' => $page->title,
			'href' => $page->getURL(),
		));
