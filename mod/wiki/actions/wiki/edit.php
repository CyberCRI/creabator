...
if (!$input['title']) {
	register_error(elgg_echo('wiki:error:no_title'));
	forward(REFERER);
}
...
if ($page_guid) {
	...
	if (!$page || !$page->canEdit()) {
		register_error(elgg_echo('wiki:error:no_save'));
		forward(REFERER);
	}
	$new_page = false;
} else {
...
if ($page->save()) {
	...
	system_message(elgg_echo('wiki:saved'));
	...
	register_error(elgg_echo('wiki:error:notsaved'));
	forward(REFERER);
}
