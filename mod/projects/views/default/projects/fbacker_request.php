...
if (!empty($vars['requests']) && is_array($vars['requests'])) {
	...
	foreach ($vars['requests'] as $user) {
		...
		$url = "action/projects/addfbacker?user_guid={$user->guid}&project_guid={$vars['entity']->guid}";
		...
		$url = 'action/projects/killfbacker?user_guid=' . $user->guid . '&project_guid=' . $vars['entity']->guid;
		$delete_button = elgg_view('output/confirmlink', array(
				'href' => $url,
				'confirm' => elgg_echo('projects:backerrequest:remove:check'),
