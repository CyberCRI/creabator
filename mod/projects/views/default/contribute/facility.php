...
$total=count($guids);
while($i<$total){
	$ud=Undone_contribute($guids[$i], 'tool');
	if($ud){
	$undones_tasks=$ud;
...
if($undones){
	...
	foreach ($undones as $undone){
		...
		$list_link=elgg_view('output/url',array('href'=>"projects/lend/{$guid}?id={$undone->id}",'text'=>'LendUs','id'=>"del-{$undone->id}",'class'=>'blue-button fr'));
